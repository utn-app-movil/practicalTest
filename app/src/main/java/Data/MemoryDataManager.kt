...
object MemoryDataManager : IDataManager {
    ...
    override fun addTournament(item: Tournament): Boolean {
        if (tournaments.any { it.name.equals(item.name, ignoreCase = true) }) return false
    ...
    override fun updateTournament(item: Tournament): Boolean {
        ...
        // Evitar duplicado por nombre (permitiendo el mismo registro)
        if (tournaments.any { it.name.equals(item.name, true) && it.id != item.id }) return false
        tournaments[idx] = item
        return true
    }
    ...
    override fun addTraining(item: Training): Boolean {
        if (trainings.any { it.name.equals(item.name, ignoreCase = true) }) return false
    ...
    override fun addTeam(item: Team): Boolean {
        if (teams.any { it.name.equals(item.name, ignoreCase = true) }) return false
